...
class SqliteStore {
    constructor({filename} = {}) {
        ...
        this.db = new sqlite3.Database(filename, (err) => {
            if(err) throw new Error(`Failed to connect to SQLite database: ${err.message}`);

            this.db.run(createTableSQL, (err) => {
                if(err) throw new Error(`Failed to create table: ${err.message}`);
            });
        });
    }
    ...
    async extract(options) {
        return new Promise((resolve, reject) => {
            this.db.get(`SELECT data FROM wsp_sessions WHERE session_name = ?`, [options.session], (err, row) => {
                if(err) return reject(err);
                if(row) fs.writeFileSync(options.path, row.data);
                resolve();
            });
        });
    }
